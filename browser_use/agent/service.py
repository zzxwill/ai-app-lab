...
class Agent(Generic[Context]):
    ...
    def __init__(
            ...
            register_new_step_callback: Callable[[
                'BrowserState', 'AgentOutput', int], Awaitable[None]] | None = None,
            register_new_progress_callback: Callable[[
                str], Awaitable[None]] | None = None,
            register_done_callback: Callable[[
                'AgentHistoryList'], Awaitable[None]] | None = None,
        ...
        self.register_new_progress_callback = register_new_progress_callback
    ...
    async def step(self, step_info: Optional[AgentStepInfo] = None) -> None:
        ...
        try:
            ...
            try:
                if self.register_new_progress_callback:
                    await self.register_new_progress_callback(f"Start planning step {self.state.n_steps}...")
            ...
            if self.register_new_progress_callback:
                await self.register_new_progress_callback("Start taking actions...")
            ...
            self.telemetry.capture(
                AgentStepTelemetryEvent(
                    ...
                    step_error=[r.error for r in result if r.error] if result else [
                        'No result'],
                )
            )
            if not result:
    ...
    def _make_history_item(
        ...
        history_item = AgentHistory(
            model_output=model_output, result=result, state=state_history, metadata=metadata)
    ...
    async def multi_act(
        ...
        for i, action in enumerate(actions):
            if self.register_new_progress_callback:
                await self.register_new_progress_callback(f"Taking action #{i+1}: {action.model_dump(exclude_none=True)}")
    ...
    async def _run_planner(self) -> Optional[str]:
        ...
        planner_messages = [
            PlannerPrompt(
                self.controller.registry.get_prompt_description()).get_system_message(),
