...
from config import ARK_API_KEY, API_ADDR
...
def stream_chat(
    ...
    thinking_msg = ChatMessage(
        content=sum_reasoning_content,
        metadata={
            "title": f"🤔 thinking round {planning_rounds}",
            "id": f"thinking-round-{planning_rounds}",
            "status": "pending",
        },
    )

    searching_msg = ChatMessage(
        content="",
        metadata={
            "title": f"🔍 searching round {planning_rounds}",
            "id": f"searching-round-{planning_rounds}",
            "status": "pending",
        },
    )
    ...
    for rsp in client.chat.completions.create(
        ...
        # round vars
        reasoning_content = (
            rsp.choices[0].delta.reasoning_content
            if hasattr(rsp.choices[0].delta, "reasoning_content")
            else ""
        )
        content = (
            rsp.choices[0].delta.content
            if hasattr(rsp.choices[0].delta, "content")
            else ""
        )
