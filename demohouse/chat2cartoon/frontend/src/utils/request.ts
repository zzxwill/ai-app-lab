...
export const getDEFAULT_ERROR_MESSAGE = () =>
  '系统异常，请联系客服或重试';
  ...
  if (isAxiosError(response)) {
    ...
    if (response.response) {
      ...
      if (!responseMetadata) {
        ...
        const rawHttpStatus = response.response.headers?.['x-real-status'] ?? response.response.status;
...
const request: Request = async (params, options = {}) => {
  const { data = {}, url, projectName, projectNameKey = 'ProjectName' } = params;
